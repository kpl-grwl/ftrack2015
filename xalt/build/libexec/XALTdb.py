...
class XALTdb(object):
  ...
  def link_to_db(self, reverseMapT, linkT):
    ...
    try:
      ...
      self.load_function(conn, linkT['func_list'], "join_function_link", link_id)  #ftrack2015
...
#                                                               ftrack2015 Starts >>  
  def load_function(self, conn, objA, tableName, index):
    """
    Store the functions list that an executable calls into the XALT DB
    @param conn:         The db connection object
    @param objA:         The array of functions that are stored
    @param tableName:    Name of the join table
    @param index:        The db index of the join table    
    """
	...
	query = "SELECT func_id FROM xalt_function WHERE function_name='%s'" % (func_name)
	conn.query(query)
	result = conn.store_result()
	if (result.num_rows() > 0):
	  ...
	  query = "INSERT into xalt_function VALUES (NULL, '%s')" % (func_name)
	  conn.query(query)
	  func_id = conn.insert_id()

        # Now join link_id with func_id:
	query = "INSERT into %s VALUES (NULL, '%d', '%d') " % (tableName, func_id, index)
	conn.query(query)
	query = "COMMIT"
	conn.query(query)
      ...
      print(XALT_Stack.contents())
      print(query)
      print("load_xalt_functions(): Error %d: %s" % (e.args[0], e.args[1]))
      sys.exit (1)
